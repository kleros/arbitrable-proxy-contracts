...
contract ArbitrableProxy is IDisputeResolver {
    ...
    struct Round {
        ...
        mapping(uint => bool) hasPaid; // True this ruling option was fully funded has fully paid its fee. False otherwise.
    ...
    function fundAppeal(uint _localDisputeID, uint _ruling) external override payable returns (bool fullyFunded){
        ...
        if (winner == _ruling){
            ...
            require(now-appealPeriodStart < (appealPeriodEnd-appealPeriodStart)/2, "The loser must contribute during the first half of the appeal period.");
        }
    ...
    function getOpenDisputes(uint _cursor, uint _count) external view returns (uint[] memory openDisputes, bool hasMore)
    {
        ...
        for (uint i = _cursor; i < disputes.length && (noOfOpenDisputes < _count || 0 == _count); i++) {
